...
# ── DDPM Model ────────────────────────────────────────────────────────────────
# Replaces DALL-E image generation with the model trained in the notebook.
# Expects score_model.pth saved in the same directory as this file.
...
_score_model = None
try:
    _score_model = SimpleUNet(
        base_ch=64, emb_dim=64, num_classes=NUM_CLASSES, in_channels=IN_CHANNELS
    ).to(DEVICE)
    _score_model.load_state_dict(
        torch.load(os.path.join(BASE_DIR, "score_model.pth"), map_location=DEVICE)
    )
    _score_model.eval()

    # Pre-compute sampling buffers (same values as notebook)
    _betas           = torch.linspace(0.0001, 0.02, TIMESTEPS, device=DEVICE)
    _alphas          = 1. - _betas
    _alphas_bar      = torch.cumprod(_alphas, dim=0)
    _alphas_bar_prev = torch.cat([torch.tensor([1.0], device=DEVICE), _alphas_bar[:-1]], dim=0)
    _sqrt_alphas_inv = torch.sqrt(1.0 / _alphas)
    _coef_eps        = (1 - _alphas) / torch.sqrt(1.0 - _alphas_bar)
    _posterior_var   = (1 - _alphas) * (1 - _alphas_bar_prev) / (1 - _alphas_bar)

    print(f"DDPM model loaded on {DEVICE}.")
except Exception as e:
    print(f"Warning: Could not load DDPM model: {e}")


@torch.no_grad()
def _ddpm_sample(class_label: int = 0):
    """Run the reverse diffusion process to generate one image."""
    x_t    = torch.randn(1, IN_CHANNELS, IMG_SIZE, IMG_SIZE, device=DEVICE)
    labels = torch.full((1,), class_label, device=DEVICE, dtype=torch.long)
    for t_val in reversed(range(TIMESTEPS)):
        t    = torch.full((1,), t_val, device=DEVICE, dtype=torch.long)
        mean = (x_t - _coef_eps[t_val] * _score_model(x_t, t, labels)) * _sqrt_alphas_inv[t_val]
        if t_val > 0:
            x_t = mean + torch.sqrt(_posterior_var[t_val]) * torch.randn_like(x_t)
        else:
            x_t = mean
    return x_t
...
def generate_image(title: str, style: str) -> str:
    """Generates one image using the trained DDPM model, saves to disk, returns filename."""
    class_label = abs(hash(style)) % NUM_CLASSES
    img_tensor  = _ddpm_sample(class_label)
    img_tensor  = (img_tensor.clamp(-1, 1) + 1) / 2.0  # unnormalize to [0, 1]

    img_np = img_tensor[0].cpu().numpy()
    pil    = PILImage.fromarray(
        (img_np.squeeze(0) * 255).astype(np.uint8), mode='L'
    ).convert('RGB')
    pil = pil.resize((512, 512), PILImage.NEAREST)
...
def extract_image_style_from_feedback(feedback: str, original_style: str) -> str:
    """
    Extracts a new style description from the user's image feedback.
